 ...
 *                   danger  → red confirm button  (delete / remove)
 *                   logout  → navy confirm button  (sign out)
 *                   default → navy confirm button
 */
    ...
    <div className="cm-overlay" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="cm-title">
      <div className={`cm-modal cm-modal--${variant}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="cm-actions">
          ...
          <button className={`cm-btn cm-btn--confirm cm-btn--confirm-${variant}`} onClick={onConfirm}>
            {confirmLabel}
          </button>
        </div>
      </div>
