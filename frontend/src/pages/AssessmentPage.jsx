...
// ── FINAL CODE — uncomment when backend is ready — DO NOT DELETE
...
export default function AssessmentPage() {
  ...
  */
  // Mock — DELETE after backend is ready
  useEffect(() => {
    setStudents(MOCK_STUDENTS);
    setLoadingStudents(false);
  }, []);

  // ── Load all passages when language changes ───────────────
  // ── FINAL CODE — uncomment when backend is ready — DO NOT DELETE
  /*
  useEffect(() => {
    ...
    passagesApi
      ...
      .then((data) => setAllPassages(data.filter((p) => !p.is_archived)))
      .catch((e) => setFetchError(e.response?.data?.detail || e.message))
  ...
  */
  ...
  useEffect(() => {
    setLoadingPassages(true);
    const t = setTimeout(() => {
      setAllPassages(MOCK_PASSAGES.filter((p) => p.language === form.language));
      setLoadingPassages(false);
    }, 300);
    return () => clearTimeout(t);
  }, [form.language]);

  // ── Recording timer (stops at 2 min) ─────────────────────
  useEffect(() => {
  ...
  function validate() {
    ...
    if (!form.passage_id) { setCreateError("Please select a passage."); return false; }
    return true;
  }

  // ── Continue from step 1 → create session ────────────────
  // ── FINAL CODE — uncomment when backend is ready — DO NOT DELETE
  /*
